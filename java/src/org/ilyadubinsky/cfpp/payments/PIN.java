...
public class PIN {
	 ...
	 */
	public static byte[] makePINBlock(int format, @NonNull byte[] pin, byte[] pan, byte[] uniqueID) {
		if (format < 0 || format > 4 || pin.length == 0 || pin.length > PIN_MAX_LENGTH)
			return null;
		...
		case 1:
			if (uniqueID != null) {
				...
				p += padLen;
				log.fine(
						" Block 1: " + IO.printByteArray(BitOps.packBCD(unpackedBuffer, PIN_BLOCK_SIZE_0123_B, false)));
	 ...
	 */
	public static byte[] encryptPINBlock4(@NonNull byte[] key, @NonNull byte[] rawPINBlock4)
	 ...
	 * @throws BadPaddingException
	 */
	public static byte[] decryptPINBlock4(@NonNull byte[] key, @NonNull byte[] epb, @NonNull byte[] pan)
	...
	public static byte[] encryptKeyVariant(@NonNull byte[] key, @NonNull byte[] kek, @NonNull byte[] variant)
		 ...
		 */
		if (variant.length <= key.length / Constants.DES_KEY_SIZE_1_B)
			return null;
		...
		for (int i = 0; i < key.length / Constants.DES_KEY_SIZE_1_B; i++) {
			...
			/*
			 * XOR variant byte with the first byte of KEK's second half again to undo the
			 * first XOR
			 */
			kek[Constants.DES_KEY_SIZE_1_B] ^= variant[i];
		}
