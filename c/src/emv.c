...
static CA_KEY ca_key_table[] = { { 0x01, { 0xc3, 0x39, 0xcd, 0x81, 0x3e, 0xbe,
		...
		0x2f, 0xab }, 248, 0x03, { 0x82, 0x26, 0x89, 0x00, 0xd4, 0x7f, 0x1d,
		...
		0x8b }, 248 }, { 0x92,
		{  }, 176, 0x03, { 0 }, 0 }, { 0xFA,
		{  }, 144, 0x03, { 0 }, 0 } };
...
static int emv_derive_icc_master_key_aes(uint8_t *unpacked_input,
		size_t unpacked_input_len, uint8_t *encryption_key,
		size_t encryption_key_len, uint8_t *output, size_t output_len);

static int emv_derive_icc_master_key_des(uint8_t *unpacked_input,
		size_t unpacked_input_len, uint8_t *encryption_key, uint8_t *output,
		size_t output_len);

static int emv_compute_3des_mac(uint8_t *data, size_t data_len, uint8_t *key,
		uint8_t *output, size_t output_len);

static int emv_compute_aes_cmac(uint8_t *data, size_t data_len, uint8_t *key,
...
void print_issuer_pk_details_header(ISSUER_PK_DETAILS_HEADER *header) {
	...
	print_array("\t\tIssuer identifier (right-padded with FF): ",
			header->issuer_identifier, 4, "\n");
	print_array("\t\tCertificate expiration MMYY: ",
			header->certificate_expiration, 2, "\n");
...
void print_icc_pk_details_header(ICC_PK_DETAILS_HEADER *header) {
	...
	printf("\t\tCertificate format (always 04): 0x%02X\n",
			header->certificate_format);
	print_array("\t\tApplication PAN(right-padded with 0xF): ",
			header->applicaton_pan, 10, "\n");
	print_array("\t\tCertificate expiration MMYY: ",
			header->certificate_expiration, 2, "\n");
	print_array("\t\tCertificate serial: ", header->certificate_serial, 3,
			"\n");
...
int emv_recover_issuer_public_key(uint8_t ca_pk_idx, uint8_t *issuer_pk_cert,
	...
	uint8_t *hash_function_input = calloc(
			EMV_MAX_CA_KEY_SIZE + issuer_pk_remainder_len, sizeof(uint8_t));
	...
	rsa = make_rsa_key(ca_pk->modulus, ca_pk->modulus_len,
			ca_pk->public_exponent, NULL, 0);
	...
	int recovered_size = RSA_public_decrypt(issuer_pk_cert_len, issuer_pk_cert,
	...
	/* combine the key */
	/* copy the first part */
	memcpy(combined_key, to + sizeof(ISSUER_PK_DETAILS_HEADER),
			recovered_size - SHA_DIGEST_LENGTH
					- sizeof(ISSUER_PK_DETAILS_HEADER) - 1);
	memcpy(
			combined_key + recovered_size - SHA_DIGEST_LENGTH
					- sizeof(ISSUER_PK_DETAILS_HEADER) - 1, issuer_pk_remainder,
			issuer_pk_remainder_len);
	...
	if (issuer_pk_remainder_len > 0) {
		memcpy(hash_function_input + hash_buffer_ptr, issuer_pk_remainder,
				issuer_pk_remainder_len);
	...
	if (header->issuer_pk_exponent_len == 1) {
		...
		memcpy(hash_function_input + hash_buffer_ptr,
				((uint8_t*) &network_order) + 1, 3);
	...
	print_array("\tKey hash as computed: ", hash_buffer, SHA_DIGEST_LENGTH,
			"\n");
	print_array("\tKey hash as provided: ",
			to + recovered_size - SHA_DIGEST_LENGTH - 1, SHA_DIGEST_LENGTH,
			"\n");

	if (memcmp(hash_buffer, to + recovered_size - SHA_DIGEST_LENGTH - 1,
			SHA_DIGEST_LENGTH)) {
	...
	cleanup: free(hash_function_input);
	RSA_free(rsa);
	return retval;
}
...
int emv_recover_icc_key(uint8_t *icc_cert, size_t icc_cert_len,
		...
		uint8_t *ipk, size_t ipk_len, uint32_t ipk_exponent, uint8_t *output,
		ICC_PK_DETAILS_HEADER *header, uint8_t *static_data,
	...
	if (!(decrypted_cert_len = RSA_public_decrypt(icc_cert_len, icc_cert,
			decrypted_cert, rsa, RSA_NO_PADDING)))
		goto cleanup;
	...
	size_t to_copy =
			header->icc_pk_len > EMV_MAX_ICC_KEY_LEN ?
					EMV_MAX_ICC_KEY_LEN : header->icc_pk_len;
	...
	size_t hash_input_len = sizeof(ICC_PK_DETAILS_HEADER) - 1
			...
			+ header->icc_pk_exponent_len + static_data_len;
	size_t hash_ptr = 0;
	...
	memcpy(hash_input + hash_ptr,
			decrypted_cert + sizeof(ICC_PK_DETAILS_HEADER),
			decrypted_cert_len - EMV_HASH_SIZE - 1
					- sizeof(ICC_PK_DETAILS_HEADER));

	hash_ptr += /* header->icc_pk_len */decrypted_cert_len - EMV_HASH_SIZE - 1
			- sizeof(ICC_PK_DETAILS_HEADER);
	...
	print_array("\tValue as provided:    ",
			decrypted_cert + decrypted_cert_len - EMV_HASH_SIZE - 1,
			EMV_HASH_SIZE, "\n");
 ...
 */
int emv_derive_icc_master_key(uint8_t *unpacked_pan, size_t unpacked_pan_len,
		uint8_t *unpacked_csn, uint8_t *encryption_key,
		size_t encryption_key_len, int algorithm, uint8_t *output,
		size_t output_len) {
	if (!(unpacked_pan && unpacked_pan_len && output && output_len
	...
	if (ALGORITHM_TDES == algorithm) {
		if (key_size != TDES_KEY_LENGTH_2)
			return EMV_ERROR;
	} else if (ALGORITHM_AES == algorithm) {
		if (!VALID_AES_KEY_SIZE(key_size))
			return EMV_ERROR;
	}
	...
	if (ALGORITHM_AES == algorithm) {
		return emv_derive_icc_master_key_aes(unpacked_input,
				unpacked_pan_len + CSN_LENGTH, encryption_key,
				encryption_key_len, output, output_len);
	} else {
		return emv_derive_icc_master_key_des(unpacked_input,
				unpacked_pan_len + CSN_LENGTH, encryption_key, output,
				output_len);
	}
 ...
 */
int emv_sign_issuer_public_key(uint8_t ca_index, uint8_t *issuer_pk,
		uint32_t issuer_pk_exponent, ISSUER_PK_DETAILS_HEADER *issuer_details,
		uint8_t *output_cert, size_t *output_cert_len,
		uint8_t *output_remainder, size_t *output_remainder_len) {

	/* validate the input */
	if (!(issuer_pk && issuer_details && output_cert && output_cert_len
	...
	size_t issuer_pk_len_used =
			issuer_details->issuer_pk_len > EMV_MAX_ISS_KEY_LEN ?
					EMV_MAX_ISS_KEY_LEN : issuer_details->issuer_pk_len;
	...
	*output_cert_len = EMV_MAX_CA_KEY_SIZE;
	...
	if (issuer_pk_len_used < EMV_MAX_ISS_KEY_LEN) {
		/* pad with 0xBB */
		memset(result + result_ptr, EMV_PK_PADDING,
				EMV_MAX_ISS_KEY_LEN - issuer_pk_len_used);
		...
		*output_remainder_len = issuer_pk_len_used - EMV_MAX_ISS_KEY_LEN;
		if (*output_remainder_len > 0)
			memcpy(output_remainder, issuer_pk + issuer_pk_len_used,
					*output_remainder_len);
		print_array("\tKey remainder: ", output_remainder,
				*output_remainder_len, "\n");
	}
	...
	/* copy with padding */
	memcpy(hash_input, result + 1,
			sizeof(ISSUER_PK_DETAILS_HEADER) - 1 + EMV_MAX_ISS_KEY_LEN);
	...
	SHA1(hash_input, hash_ptr,
			result + EMV_MAX_CA_KEY_SIZE - 1 - EMV_HASH_SIZE);

	print_array("\tHash output: ",
			result + EMV_MAX_CA_KEY_SIZE - 1 - EMV_HASH_SIZE, EMV_HASH_SIZE,
			"\n");
	...
	rsa = make_rsa_key(ca_key->modulus, ca_key->modulus_len,
			ca_key->public_exponent, ca_key->private_exponent,
			ca_key->private_exponent_len);

	if (!rsa)
		goto cleanup;

	int actual_len = RSA_private_encrypt(EMV_MAX_CA_KEY_SIZE, result,
			output_cert, rsa, RSA_NO_PADDING);
	if (actual_len < 0)
		goto cleanup;
...
int emv_sign_static_data(uint8_t *issuer_pub_key, size_t issuer_pub_key_len,
		uint32_t issuer_pk_exponent, uint8_t *issuer_priv_key,
		size_t issuer_priv_key_len, uint8_t *auth_data, size_t auth_data_len,
		SDA_DETAILS_HEADER *sda_details, uint8_t *output) {
	int retval = 0;

	RSA *rsa = NULL;

	/* validate the inputs */
	if (!(issuer_pub_key && issuer_priv_key_len && issuer_pk_exponent
			&& issuer_priv_key && issuer_priv_key_len && auth_data
			&& auth_data_len && output))
		return EMV_ERROR;
	/* allocate buffer for the hash function input. The size is roughly the issuer modulus length plus the extra data */
	uint8_t *buffer = calloc(issuer_pub_key_len + auth_data_len,
			sizeof(uint8_t));
	size_t hash_ptr = 0;
	...
	memset(buffer + hash_ptr, EMV_PK_PADDING,
			issuer_pub_key_len - sizeof(SDA_DETAILS_HEADER) - EMV_HASH_SIZE
					- 1);
	...
	rsa = make_rsa_key(issuer_pub_key, issuer_pub_key_len, issuer_pk_exponent,
			issuer_priv_key, issuer_priv_key_len);
...
int emv_sign_dynamic_data(uint8_t *icc_pub_key, size_t icc_pub_key_len,
		uint32_t icc_pk_exponent, uint8_t *icc_priv_key,
		size_t icc_priv_key_len, uint8_t *icc_data, uint8_t *term_data,
		size_t term_data_len, DDA_DETAILS_HEADER *dda_details, uint8_t *output) {
	int retval = EMV_ERROR;

	RSA *rsa = NULL;

	/* validate the inputs */
	if (!(icc_pub_key && icc_pub_key_len && icc_pk_exponent && icc_priv_key
			&& icc_priv_key_len && term_data && term_data_len && icc_data
			&& dda_details && output))
		return EMV_ERROR;
	...
	uint8_t *buffer = calloc(
			icc_pub_key_len + dda_details->dd_len + term_data_len,
			sizeof(uint8_t));
	size_t hash_ptr = 0;
	...
	memset(buffer + hash_ptr, EMV_PK_PADDING,
			icc_pub_key_len - sizeof(DDA_DETAILS_HEADER) - dda_details->dd_len
					- EMV_HASH_SIZE - 1);
	hash_ptr += icc_pub_key_len - sizeof(DDA_DETAILS_HEADER)
			- dda_details->dd_len - EMV_HASH_SIZE - 1;
	...
	/* encrypt */
	print_array("\t\tEncryption input: ", buffer, hash_ptr, "\n");
	rsa = make_rsa_key(icc_pub_key, icc_pub_key_len, icc_pk_exponent,
			icc_priv_key, icc_priv_key_len);
 ...
 */
int emv_validate_dda(uint8_t *dda, size_t dda_len, uint8_t *icc_pub_key,
		size_t icc_pub_key_len, uint32_t icc_pk_exponent, uint8_t *term_data,
		size_t term_data_len, DDA_DETAILS_HEADER *header) {
	int retval = EMV_ERROR;
	RSA *rsa = NULL;
	/* validate the inputs */
	...
	int actual_len = RSA_public_decrypt(icc_pub_key_len, dda, buffer, rsa,
			RSA_NO_PADDING);
	if (actual_len < 0)
		goto cleanup;
	...
	memcpy(hash_provided, buffer + icc_pub_key_len - 1 - EMV_HASH_SIZE,
			EMV_HASH_SIZE);
	print_array("\t\tDecrypted value: ", buffer, actual_len, "\n");
	/* copy the terminal data into the buffer */
	memcpy(buffer + icc_pub_key_len - 1 - EMV_HASH_SIZE, term_data,
			term_data_len);

	/* compute the SHA of the data */
	print_array("\t\tHash input:      ", buffer + 1,
			icc_pub_key_len - 2 - EMV_HASH_SIZE + term_data_len, "\n");
	uint8_t hash_computed[EMV_HASH_SIZE];
	SHA1(buffer + 1, icc_pub_key_len - 2 - EMV_HASH_SIZE + term_data_len,
			hash_computed);
 ...
 */
int emv_validate_sda(uint8_t *sda, uint8_t *issuer_pk, size_t issuer_pk_len,
		uint32_t issuer_exponent, uint8_t *auth_data, size_t auth_data_len,
		SDA_DETAILS_HEADER *header) {
	int retval = EMV_ERROR;
	RSA *rsa = NULL;
	/* validate the inputs */
	if (!(sda && issuer_pk && issuer_pk_len && issuer_exponent && auth_data
			&& auth_data_len && header))
		return EMV_ERROR;
	...
	int actual_len = RSA_public_decrypt(issuer_pk_len, sda, buffer, rsa,
			RSA_NO_PADDING);
	if (actual_len < 0)
		goto cleanup;
	...
	/* copy away the provided SHA */
	uint8_t hash_provided[EMV_HASH_SIZE];
	memcpy(hash_provided, buffer + issuer_pk_len - 1 - EMV_HASH_SIZE,
			EMV_HASH_SIZE);
	print_array("\t\tDecrypted value: ", buffer, actual_len, "\n");
	/* copy the additional data into the buffer */
	memcpy(buffer + issuer_pk_len - 1 - EMV_HASH_SIZE, auth_data,
			auth_data_len);
	/* compute the SHA of the data */
	print_array("\t\tHash input:      ", buffer + 1,
			issuer_pk_len - 2 - EMV_HASH_SIZE + auth_data_len, "\n");
	uint8_t hash_computed[EMV_HASH_SIZE];
	SHA1(buffer + 1, issuer_pk_len - 2 - EMV_HASH_SIZE + auth_data_len,
			hash_computed);
	print_array("\t\tHash provided:   ", hash_provided, EMV_HASH_SIZE, "\n");
	print_array("\t\tHash computed:   ", hash_computed, EMV_HASH_SIZE, "\n");
...
int emv_sign_icc_public_key(uint8_t *issuer_pub_key, size_t issuer_pub_key_len,
		uint32_t issuer_pk_exponent, uint8_t *issuer_priv_key,
		size_t issuer_priv_key_len, uint8_t *icc_pk, uint32_t icc_pk_exponent,
		ICC_PK_DETAILS_HEADER *icc_details, uint8_t *auth_data,
		size_t auth_data_len, uint8_t *output_cert, uint8_t *output_remainder,
	...
	/* prepare the buffer */
	uint8_t *buffer = calloc( EMV_MAX_ISS_KEY_LEN + auth_data_len,
			sizeof(uint8_t));
	size_t buffer_ptr = 0;
	...
	/* maximum length */
	size_t icc_pk_cert_max_len = issuer_pub_key_len - EMV_HASH_SIZE
			- sizeof(ICC_PK_DETAILS_HEADER) - 1;
	/* actual length */
	size_t icc_pk_cert_len;

	if (icc_pk_len > icc_pk_cert_max_len) {
		/* there is a remainder */
		memcpy(output_remainder, icc_pk + icc_pk_cert_max_len,
				icc_pk_len - icc_pk_cert_max_len);
	...
	memset(buffer + buffer_ptr + icc_pk_cert_len, EMV_PK_PADDING,
			icc_pk_cert_max_len - icc_pk_cert_len);
	...
	if (!SHA1(buffer + 1, buffer_ptr - 1, signature))
		goto cleanup;
	...
	retval = RSA_private_encrypt(buffer_ptr, buffer, output_cert, rsa,
			RSA_NO_PADDING);
	if (retval < 0)
		goto cleanup;
	...
	cleanup: ERR_print_errors_fp(stdout);
	free(buffer);
	if (rsa)
		RSA_free(rsa);
	return retval;
...
static int emv_derive_icc_master_key_aes(uint8_t *unpacked_input,
	...
	pack_bcd(unpacked_input, unpacked_input_len, packed_input, AES_BLOCK_SIZE,
			PAD_LEFT);
	print_array("\tPacked input: ", packed_input, AES_BLOCK_SIZE, "\n");

	/* prepare the first chunk of the output */
	AES_KEY key;
	if (AES_set_encrypt_key(encryption_key, encryption_key_len * 8, &key))
		goto cleanup;
	...
	if (output_len > AES_BLOCK_SIZE) {
		...
		memcpy(output + AES_BLOCK_SIZE, output_buffer,
...
static int emv_derive_icc_master_key_des(uint8_t *unpacked_input,
	...
	if (unpacked_input_len <= EMV_OPTION_A_MAX_PAN_LEN + CSN_LENGTH) /* Option A */{
		if (unpacked_input_len <= TDES_BLOCK_SIZE << 1)
			...
			pack_bcd(unpacked_input, unpacked_input_len, enc_input,
					TDES_BLOCK_SIZE, PAD_RIGHT);
		else {
			/* pack while skipping the extra digits from the left */
			pack_bcd(
					unpacked_input
							+ (unpacked_input_len - (TDES_BLOCK_SIZE << 1)),
					TDES_BLOCK_SIZE << 1, enc_input, TDES_BLOCK_SIZE,
					PAD_RIGHT);
		}
	} else {
		/* pack with left padding by 1 nibble if odd */
		pack_bcd(unpacked_input, unpacked_input_len, enc_input,
				(unpacked_input_len + 1) >> 1, PAD_LEFT);
		print_array("\tPacked input: ", enc_input,
				(unpacked_input_len + 1) >> 1, "\n");
		uint8_t hash_output[SHA_DIGEST_LENGTH];
		SHA1((unsigned char*) enc_input, (unpacked_input_len + 1) >> 1,
		...
		decimalize_vector(hash_output, 2 * SHA_DIGEST_LENGTH,
				hash_decimalized_unpacked, TDES_BLOCK_SIZE << 1);

		memset(enc_input, 0, TDES_BLOCK_SIZE << 1);
		pack_bcd(hash_decimalized_unpacked, TDES_BLOCK_SIZE << 1, enc_input,
				TDES_BLOCK_SIZE, PAD_RIGHT);
	...
	DES_set_key_unchecked(
			(const_DES_cblock*) (encryption_key + (TDES_KEY_LENGTH_1)), &key_b);

	DES_ecb2_encrypt((DES_cblock* ) enc_input, (DES_cblock* )output, &key_a,
			&key_b, DES_ENCRYPT);
	error_code = ERR_get_error();
	if (error_code)
		return EMV_ERROR;
	...
	DES_ecb2_encrypt((DES_cblock* ) enc_input,
			(DES_cblock*) (output+TDES_BLOCK_SIZE), &key_a, &key_b,
			DES_ENCRYPT);
	error_code = ERR_get_error();
	if (error_code)
		return EMV_ERROR;
...
int emv_derive_icc_session_key(uint8_t *icc_master_key,
		size_t icc_master_key_length, int algorithm, uint8_t *atc,
		uint8_t *output, size_t output_len) {
	...
	if (ALGORITHM_TDES == algorithm) {
		...
		DES_set_key_unchecked(
				(const_DES_cblock*) (icc_master_key + (TDES_KEY_LENGTH_1)),
				&des_key_b);
	} else { /* assuming it can only be AES */
		AES_set_encrypt_key(icc_master_key, icc_master_key_length << 3,
				&aes_key);
	}
	...
	for (int i = 0; i < iters; i++) {
		...
		print_array("\tInput data for encryption: ", input_data,
				TDES_BLOCK_SIZE, "\n");
		if (algorithm == ALGORITHM_TDES) {

			DES_ecb2_encrypt((DES_cblock* ) input_data,
					(DES_cblock*)(output_data + (i*TDES_BLOCK_SIZE)),
					&des_key_a, &des_key_b, DES_ENCRYPT);

			print_array("\tEncryption output: ",
					output_data + (i * TDES_BLOCK_SIZE), TDES_BLOCK_SIZE, "\n");
			...
			AES_ecb_encrypt(input_data, output_data + (i * AES_BLOCK_SIZE),
					&aes_key, AES_ENCRYPT);
			print_array("\tEncryption output: ",
					output_data + (i * AES_BLOCK_SIZE), AES_BLOCK_SIZE, "\n");
			error_code = ERR_get_error();
			if (error_code)
				goto cleanup;
		}
...
int emv_generate_arqc(uint8_t *session_key, size_t session_key_len,
	...
	/* Validate inputs */
	if (!(session_key && session_key_len && arqc_data && arqc_data_len && output
			&& output_len))
		return result;
	...
	if (ALGORITHM_AES == algorithm) {
		if (!emv_compute_aes_cmac(arqc_data, arqc_data_len, session_key,
				session_key_len, output, output_len))
			goto cleanup;
	} else {
		if (!emv_compute_3des_mac(arqc_data, arqc_data_len, session_key, output,
				output_len))
			goto cleanup;
	}
...
int emv_generate_arpc(uint8_t *arqc, uint8_t *arc, uint8_t *csu, uint8_t *pad,
	...
	if (ALGORITHM_AES == algorithm && !(VALID_AES_KEY_SIZE(key_len)))
		return EMV_ERROR;
	if (ALGORITHM_TDES == algorithm && (TDES_KEY_LENGTH_2 != key_len))
		return EMV_ERROR;
	...
	if (arc) {
		...
		if (ALGORITHM_TDES == algorithm) {
			...
			DES_set_key_unchecked((const_DES_cblock*) (key + TDES_KEY_LENGTH_1),
					&des_key_b);
			DES_ecb2_encrypt((const_DES_cblock* ) buffer, (DES_cblock* )output,
					&des_key_a, &des_key_b, DES_ENCRYPT);
		...
		if (ALGORITHM_TDES == algorithm) {
			emv_compute_3des_mac(buffer, input_len, key, output,
					EMV_ARQC_MIN_LEN);
		} else {
			emv_compute_aes_cmac(buffer, input_len, key, key_len, output,
					EMV_ARQC_MIN_LEN);
		}
...
static int emv_compute_aes_cmac(uint8_t *data, size_t data_len, uint8_t *key,
	...
	/* Validate inputs */
	if (!(data && data_len && key && key_len && output && output_len))
		return EMV_ERROR;
	...
	size_t outlen;
	if (!CMAC_Final(cmac_ctx, output_buffer, &outlen))
		goto cleanup;
...
static int emv_compute_3des_mac(uint8_t *data, size_t data_len, uint8_t *key,
		uint8_t *output, size_t output_len) {
	/* validate inputs */
	if (!(data && data_len && key && output && output_len))
		return EMV_ERROR;
	...
	size_t padded_data_size = TDES_BLOCK_SIZE
			* ((data_len + 1) / TDES_BLOCK_SIZE
					+ !!((data_len + 1) % TDES_BLOCK_SIZE));
	...
	memcpy(output, padding_buffer + (padded_data_size - TDES_BLOCK_SIZE),
			output_len);
	result = EMV_SUCCESS;
